...
const LANGUAGES = ['English', 'Arabic', 'French', 'German', 'Russian', 'Spanish', 'Italian'];
...
function generateGuide(id: number) {
  ...
  return {
    ...
    specializations: randomFrom(SPECIALIZATIONS, faker.number.int({ min: 1, max: 3 })),
    about: faker.lorem.paragraph(),
    tours: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }, generateTour),
  };
}
