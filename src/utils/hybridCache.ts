...
export class HybridCache {
  private memoryCache = new Map<string, { data: unknown; timestamp: number; size: number }>();
  ...
  constructor(maxEntries: number = 20, maxSizeMB: number = 5, ttlMs: number = 5 * 60 * 1000) {
  ...
  async clear(): Promise<void> {
    ...
    if (isRedisConnected()) {
      try {
        // Clear Redis keys matching pattern (optional - be careful in production)
  ...
  private setInMemory(key: string, data: unknown, size?: number): void {
    ...
    while (
      ...
      if (removed) this.totalSize -= removed.size;
