...
export const sendBookingConfirmationEmail = async (details: BookingEmailDetails): Promise<Result<{ success: boolean; message: string }, DomainError>> => {
  try {
    const mailOptions = {
    ...
    return err({ type: 'Unknown', message: `Failed to send email: ${(error as Error).message}` });
  }
};