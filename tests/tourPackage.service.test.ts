...
const mockPackages = [
  {
    ...
    included: ['Guide', 'Meals'] as ('Guide' | 'Meals' | 'Transport' | 'Tickets')[],
...
describe('tourPackageService', () => {
  describe('getAllTourPackages', () => {
    ...
    it('filters by category if provided', () => {
      const result = tourPackageService._test_getAllTourPackages(mockPackages, { category: 'adventure' });
      if (result.ok) {
    ...
    it('applies pagination if page and limit are provided', () => {
      const result = tourPackageService._test_getAllTourPackages(mockPackages, { page: 2, limit: 1 });
    ...
    it('returns empty array if no packages match category', () => {
      const result = tourPackageService._test_getAllTourPackages(mockPackages, { category: 'nonexistent' });
  ...
  describe('getTourPackageById', () => {
    it('returns the correct package by id', () => {
      const result = tourPackageService._test_getTourPackageById(mockPackages, '1');
      expect(result.ok && result.value.id === '1').toBe(true);
    });
    it('returns error result if not found', () => {
      const result = tourPackageService._test_getTourPackageById(mockPackages, 'not-exist');
