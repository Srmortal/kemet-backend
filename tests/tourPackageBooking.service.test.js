...
const adventuresMock = [{ id: "adv1", price: 200 }];
const adventureRepository = {
  findById: jest.fn(
    async (id) => adventuresMock.find((a) => a.id === id) || null
  ),
};
...
async function bookTourPackage(input) {
  const adventure = await adventureRepository.findById(input.tourPackageId);
  if (!adventure) {
    return {
      ok: false,
      error: { type: "NotFound", message: "Adventure not found" },
    };
  }
  try {
...
describe("TourPackageBookingService", () => {
  ...
  it("allows overlapping bookings for same user", async () => {
    adventureRepository.findById.mockImplementation(
      async (id) => adventuresMock.find((a) => a.id === id) || null
    );
    tourPackageBookingRepository.createBooking.mockImplementation(
      async (data) => ({
        id: `booking_${Date.now()}`,
        packageId: data.packageId,
        date: data.date,
  ...
  it("handles unavailable package (adventure not found)", async () => {
    ...
    const result = await bookTourPackage({
      ...validInput,
      tourPackageId: "not-found",
    });
  ...
  it("handles price change between selection and booking", async () => {
    ...
    if (result.ok && result.value) {
      expect(result.value.total).toBe(
        adventuresMock[0].price * validInput.numberOfPeople
      );
    }
    adventuresMock[0].price = originalPrice;
  });
  it("handles concurrency (simultaneous bookings)", async () => {
    ...
    const promises = [bookTourPackage(validInput), bookTourPackage(validInput)];
...
//# sourceMappingURL=tourPackageBooking.service.test.js.map
